...
interface FundPriceRangeProps {
    ticker: string;
    price: number | null;
    currency: string;
}

/** 52-недельный диапазон цены пая с маркером текущего значения. */
const FundPriceRange: React.FC<FundPriceRangeProps> = ({ ticker, price, currency }) => {
    const { data: candles = [], isLoading } = useFundCandles(ticker, getYearAgo());
