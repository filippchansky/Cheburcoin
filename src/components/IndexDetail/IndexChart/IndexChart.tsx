...
import { useIndexCandles } from '@/hooks/useIndices';
import { getMonthAgo, getSixMonthAgo, getYearAgo, getFiveYearsAgo } from '@/utils/dateUtils';
import { useDarkTheme } from '@/store/darkTheme';
import { getPalette } from '@/theme/palette';
import { intToRubCompact } from '@/utils/formatCurrency';
import style from './style.module.scss';

interface IndexChartProps {
    secid: string;
    /** Число знаков значения индекса (для подписи/тултипа). */
    decimals: number;
}
...
const IndexChart: React.FC<IndexChartProps> = ({ secid, decimals }) => {
    const [period, setPeriod] = useState<Period>('year');
    const { from, interval } = configByPeriod[period];
    const { data: candles = [], isLoading } = useIndexCandles(secid, from(), interval);
    ...
    const options = {
        ...
        series: [
            ...
            },
            {
                ...
                type: 'bar',
                yAxisIndex: 1,
                tooltip: { valueFormatter: (value: number) => intToRubCompact(value) },
    ...
    return (
        <section className={style.wrapper}>
            <div className={style.head}>
                <h2 className={style.title}>Значение и оборот</h2>
                <Segmented
                    value={period}
